(function($) {
  ...
  dataTableServerSide = (params) => {
    ...
    $('.table-ssr tfoot th').each(function(i) {
      ...
      const type = params.columns[i].searchable === false ? "hidden" : "text"

      params.columns[i].name = params.columns[i].field 
        ? params.columns[i].field 
        : params.columns[i].name

      $(this).html(`<input type="${type}" class="form-control" placeholder="${placeholder}" style="margin: 10px 0px; min-width: ${width}px" />`);
    });

    // DataTable
    let payload = params.args 
      ? params.args
      : {}
    ...
    dataTable.columns().every(function() {
        ...
        $('input', this.footer()).on('keyup', function(e) {
            if (e.key == "Enter" && that.search() !== this.value || e.target.value == "" && that.search() !== this.value) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        switch (result.value.status) {
          case 200:
            toastr.success('Data berhasil dihapus !', 'Sukses', { "progressBar": true, "showDuration": 500, "closeButton": true })
            setTimeout(() => {
              if (params.reload) {
            ...
            break;
          case 500:
            toastr.error('Data tidak bisa dihapus, karena sudah digunakan !', 'Gagal', { "progressBar": true, "showDuration": 500, "closeButton": true })
            break;
        }
